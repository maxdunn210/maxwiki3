...
class Team < MaxWikiActiveRecord
  ...
  def self.picklist(options = {})
    ...
    Team.find(:all, 
      :order => "levels_teams.display_order, lookups.display_order, teams.name", 
      :include => [:league, :level],
      #:joins => "INNER JOIN lookups AS league ON league.id = teams.league_id
      #           INNER JOIN lookups AS level ON level.id = teams.level_id",
  ...
  def self.find_team(params)
    ...
    if params[:level]
      team_find_conditions = ["teams.name = ? and levels_teams.name = ?", params[:team], params[:level] ]
    else
      team_find_conditions = ["teams.name = ?", params[:team] ]
    end
    team = Team.find(:first, :conditions => team_find_conditions, 
                     :order => "lookups.display_order",
                     :include => [:league, :level])
                     #:joins => "INNER JOIN lookups AS league ON league.id = teams.league_id
                    #            INNER JOIN lookups AS level ON level.id = teams.level_id")
  end
