...
class Page < MaxWikiActiveRecord
  ...
  def wiki_references_all
    ...
    ["#{self.name}_left","#{self.name}_right"].each do |side_name|
      side_page = Page.find_by_name(side_name)
      references << WikiReference.new(:page_id => side_page.id, :referenced_name => side_page.name, :link_type => WikiReference::INCLUDED_PAGE) unless side_page.nil?
    end
