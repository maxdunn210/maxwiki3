...
name = "#{Rails.root.to_s}/config/local_helper.rb"
...
include WebdavHelper
  ...
  def favicon_link_tag
    favicon_name = ['favicon.ico','favicon.gif','favicon.png'].find do |name| 
      File.exists?("#{Rails.root.to_s}/public/files/#{@wiki.name}/#{name}")
    end
  ...
  def maxwiki_stylesheet_link_tags
    ...
    unless @wiki.name.blank?
      if File.exist?("#{Rails.root.to_s}/public/files/#{@wiki.name}/site.css")
        html << stylesheet_link_tag("/files/#{@wiki.name}/site.css", :media => "all") + "\n"
      end
      if File.exist?("#{Rails.root.to_s}/public/files/#{@wiki.name}/site_print.css")
    ...
    if File.exist? "#{Rails.root.to_s}/public/stylesheets/main_print.css"
