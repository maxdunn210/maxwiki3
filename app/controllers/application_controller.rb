...
class ApplicationController < AuthorizationController
  ...
  def connect_to_model
    ...
    #  different directory for the the cached pages) change this to just "#{Rails.root.to_s}/public" 
    ...
    if @wiki.nil?
      ...
      @wiki_name = @wiki.name
      ApplicationController::page_cache_directory = "#{Rails.root.to_s}/public/cache" 
    end  
    ...
    unless @wiki.nil? 
      ...
      unless @wiki.name.blank?
        name = "#{Rails.root.to_s}/public/files/#{@wiki.name}/site_environment.rb"
        load name if File.exist?(name)
        
        name = "#{Rails.root.to_s}/public/files/#{@wiki.name}/site_helper.rb"
        require name if File.exist?(name)
      end
    end
