...
class #{table_name.classify} < ActiveRecord::Base
  ...
  def self.process_args(args)
    ...
    Dir.chdir options[:Chdir] do
      begin
#MD debug        require 'config/environment'
      rescue LoadError
        usage opts, <<-EOF
...
#{Dir.pwd} does not appear to be a Rails application root.
  ...
  def find_emails
    options = { :conditions => ['last_send_attempt < ? and status in (?, ?)', 
      Time.now.to_i - 300, EMAIL_QUEUED, EMAIL_ERROR] }
